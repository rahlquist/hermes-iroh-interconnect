...
def validate_ticket(raw: str) -> Dict[str, str]:
    """Parse a ``hermes-iroh://pair?peer=...&secret=...`` ticket offline.

    Returns a dict with ``peer_id`` and ``secret`` keys. Raises
    :class:`InvalidTicket` for anything that is not a well-formed Hermes
    ticket. This function performs no network I/O.
    """
    try:
    ...
    return {"peer_id": peer_id, "secret": secret}
